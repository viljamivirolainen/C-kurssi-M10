...
{
	if ((const unsigned int)x >= game->opts.mapWidth || (const unsigned int)y >= game->opts.mapHeight || x <= 0 || y <= 0)
		return 1;
	unsigned int n = 0;
	while (n < game->numMonsters)
	{
		if (game->monsters[n].pos.x == x && game->monsters[n].pos.y == y)
			return 1;
		n++;
	}
